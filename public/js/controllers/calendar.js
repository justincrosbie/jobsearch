window.angular.module('ngff.controllers.calendar', [])
  .controller('CalendarController', ['$scope', 'Global', 'Jobs',
	      ...
	      $scope.findJobs = function (query) {
	        Jobs.query(query, function (jobs) {
	          for ( var i=0; i<jobs.data.length; i++ ) {
			      $scope.events.push({
			        title: jobs.data[i].name,
			        start: new Date(jobs.data[i].start),
			        end: new Date(jobs.data[i].end),
			        className: ['newJob']
			      });
	          }
	        });
